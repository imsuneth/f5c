/* @file f5c.h
**
** high level interface to f5c framework - major definitions and function
*prototypes
...
//#define F5C_DEBUG_BRK 0x080 //break after the first batch //removed can be
// reused
#define F5C_SEC_PROF                                                           \
    0x100 // profile section by section (only effective on the CPU mode)
#define F5C_WR_RAW_DUMP                                                        \
    0x200 // to say if we should write the raw dump of the fast5
#define F5C_RD_RAW_DUMP                                                        \
    0x400             // to say if we should read the raw dump fof the fast5
#define F5C_SAM 0x800 // write output in SAM format (eventalign only)
#define F5C_SCALE_EVENTS                                                       \
    0x1000 // scale events to the model, rather than vice-versa (eventalign
           // only)
#define F5C_PRINT_RNAME                                                        \
    0x2000 // print read names instead of indexes (eventalign only)
#define F5C_PRINT_SAMPLES                                                      \
    0x4000 // write the raw samples for the event to the tsv output (eventalign
           // only)
...
#define WORK_STEAL                                                             \
    1 // simple work stealing enabled or not (no work stealing mean no load
      // balancing)
#define STEAL_THRESH 1 // stealing threshold for the CPU only sections
#define STEAL_THRESH_CUDA                                                      \
    0 // stealing threshold for the CPU part in a GPU accelerated workload

// set if input, processing and output are not to be interleaved (serial mode) -
// useful for debugging
// #define IO_PROC_NO_INTERLEAVE 1

//#define ALIGN_2D_ARRAY 1 //for CPU whether to use a 1D array or a 2D array
// note : (2D arrays are very slow due to mallocs when the number of threads is
// high)

#define CACHED_LOG                                                             \
    1 // if the log values of scalings and the model k-mers are cached
//#define LOAD_SD_MEANSSTDV 1 //if the sd_mean and the sd_stdv is to be loaded
...
typedef struct {
    ...
    // todo : these are required only for HAVE_CUDA (but need to change the
    // meth_main accordingly)
...
typedef struct {
    ...
    // read sequence //todo : optimise by grabbing it from bam seq. is it
    ...
    int64_t sum_bases;
    int64_t total_reads; // total number mapped entries in the bam file (after
                         // filtering based on flags, mapq etc)
    int64_t bad_fast5_file; // empty fast5 path returned by readdb, could not
                            // open fast5
...
/* cuda core data structure (allocated array pointers, mostly static data
...
#endif
#ifdef HAVE_CUDA
    int32_t *ultra_long_reads; // reads that are assigned to the CPU due to the
                               // unsuitability to process on the GPU
    double ret1;               // return value
#endif
} pthread_arg_t;

/* argument wrapper for multithreaded framework used for input/processing/output
 * interleaving */
typedef struct {
    core_t *core;
    db_t *db;
...
/* return status by the load_db - used for termination when all the data is
 * processed */
typedef struct {
    int32_t num_reads;
    int64_t num_bases;
...
/* Function prototypes for other non-major functions are in f5cmisc.h (and
 * f5cmisc.cuh for CUDA)*/
