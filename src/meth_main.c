...
(note that `pthread_processor` is the process-controller that will spawn user
specified number of processing threads - see below)
3. create the `pthread_post_processor` thread that will print the output and
free the databatch one the `pthread_processor` is done
...
a step controller if one requires to perform only upto a certain step such as
alignment, ignoring the reset
--the names of functions and variablrs starting with meth_ are confusing as it
stands for both meth and event now. should be fixed later
*/
...
static struct option long_options[] = {
    ...
    {"max-bases", required_argument, 0,
     'B'}, // 5 batchsize - number of bases loaded at once
    ...
    {"kmer-model", required_argument, 0,
     0}, // 11 custom nucleotide k-mer model file
    ...
    {"profile-cpu", required_argument, 0,
     0}, // 20 perform section by section (used for profiling - for CPU only)
    {"cuda-max-lf", required_argument, 0,
     0}, // 21 reads <= cuda-max-lf*avg_readlen on GPU, rest on CPU (only if
         // compiled for CUDA)
    {"cuda-avg-epk", required_argument, 0,
     0}, // 22 average number of events per kmer - for allocating GPU arrays
         // (only if compiled for CUDA)
    {"cuda-max-epk", required_argument, 0,
     0}, // 23 reads <= cuda_max_epk on GPU, rest on CPU (only if compiled for
         // CUDA)
    {"cuda-dev-id", required_argument, 0,
     0}, // 24 cuda device ID to run on (only if compiled for CUDA)
    {"cuda-mem-frac", required_argument, 0,
     0}, // 25 fraction of the free GPU memory to use (only if compiled for
         // CUDA)
    {"skip-ultra", required_argument, 0,
     0}, // 26 skip the ultra long reads for better load balancing
    {"ultra-thresh", required_argument, 0,
     0}, // 27 the threadshold for skipping ultra long reads
    ...
    {"summary", required_argument, 0,
     0}, // 33 summarize the alignment of each read/strand in FILE (eventalign
         // only)
    {"sam", no_argument, 0,
     0}, // 34 write output in SAM format (eventalign only)
    {"scale-events", no_argument, 0, 0}, // 35 scale events to the model, rather
                                         // than vice-versa (eventalign only)
    {"print-read-names", no_argument, 0,
     0}, // 36 print read names instead of indexes (eventalign only)
    {"samples", no_argument, 0, 0}, // 37 write the raw samples for the event to
                                    // the tsv output (eventalign only)
    {"meth-out-version", required_argument, 0,
     0}, // 38 specify the version of the tsv output for methylation
         // (call-methylation only)
    {"profile", required_argument, 0,
     'x'}, // 39 profile used to tune parameters for GPU
    {"meth-model", required_argument, 0,
     0}, // 40 custom methylation k-mer model file
    {0, 0, 0, 0}};
...
// function that processes a databatch - for pthreads when I/O and processing
// are interleaved
void *pthread_processor(void *voidargs) {
    ...
    fprintf(stderr, "[%s::%.3f*%.2f] %d Entries (%.1fM bases) processed\n",
            ...
            cputime() / (realtime() - realtime0), db->n_bam_rec,
            db->sum_bases / (1000.0 * 1000.0));
    ...
    if (core->opt.verbosity > 1) {
        fprintf(stderr, "[%s::%.3f*%.2f] Signal sent by processor thread!\n",
                __func__, realtime() - realtime0,
                cputime() / (realtime() - realtime0));
    }

    pthread_exit(0);
}

// function that prints the output and free - for pthreads when I/O and
...
void *pthread_post_processor(void *voidargs) {
    ...
    if (core->opt.verbosity > 1) {
        fprintf(stderr,
                "[%s::%.3f*%.2f] Signal got by post-processor thread!\n",
                __func__, realtime() - realtime0,
                cputime() / (realtime() - realtime0));
    }
...
void slow_fast5_warn(core_t *core) {
    ...
    if (core->db_fast5_time > core->process_db_time * 1.5) {
        INFO("%s", "Fast5 reading took more time than processing. Try "
                   "increasing --iop. See http://bit.ly/f5cperf");
    }
}
...
int meth_main(int argc, char *argv[], int8_t mode) {
    ...
    while ((c = getopt_long(argc, argv, optstring, long_options, &longindex)) >=
        ...
        if (c == 'r') {
        ...
        } else if (c == 'x') { // Set profile values. Any user-specified
                               // arguments will override the profile values.
            profilename = optarg;
            ...
            opt.cuda_block_size = atoi(
                optarg); // todo : warnining for cpu only mode, check limits
        } else if (c == 0 && longindex == 19) { // debug break
            // yes_or_no(&opt, F5C_DEBUG_BRK, longindex, optarg, 1);
            opt.debug_break = atoi(optarg);
        } else if (c == 0 &&
                   longindex ==
                       20) { // sectional benchmark todo : warning for gpu mode
            yes_or_no(&opt, F5C_SEC_PROF, longindex, optarg, 1);
        } else if (c == 0 &&
                   longindex ==
                       21) { // cuda todo : warning for cpu mode, error check
            opt.cuda_max_readlen = atof(optarg);
        } else if (c == 0 &&
                   longindex ==
                       22) { // cuda todo : warning for cpu mode, warning for
                             // dynamic cuda malloc mode, error check
            opt.cuda_avg_events_per_kmer = atof(optarg);
        } else if (c == 0 &&
                   longindex ==
                       23) { // cuda todo : warning for cpu mode, error check
            opt.cuda_max_avg_events_per_kmer = atof(optarg);
        } else if (c == 0 && longindex == 24) {
            opt.cuda_dev_id = atoi(optarg);
        } else if (c == 0 &&
                   longindex == 25) { // todo : warning for CPU mode, warning
                                      // for dynamic malloc mode
            ...
            is_ultra_thresh_set = 1;
                   ...
                   longindex == 28) { // write the raw dump of the fast5 files
            yes_or_no(&opt, F5C_WR_RAW_DUMP, longindex, optarg, 1);
        } else if (c == 0 &&
                   longindex == 29) { // read the raw dump of the fast5 files
            yes_or_no(&opt, F5C_RD_RAW_DUMP, longindex, optarg, 1);
        } else if (c == 'o') {
            if (strcmp(optarg, "-") != 0) {
                if (freopen(optarg, "wb", stdout) == NULL) {
                    ERROR("failed to write the output to file %s : %s", optarg,
                          strerror(errno));
                    exit(EXIT_FAILURE);
                }
            }
            ...
            if (opt.num_iop < 1) {
                ERROR("Number of I/O processes should be larger than 0. You "
                      "entered %d",
                      opt.num_iop);
                exit(EXIT_FAILURE);
            }
            ...
            if (mode != 1) {
                ERROR("%s",
                      "Option --scale-events is available only in eventalign");
                exit(EXIT_FAILURE);
            }
            yes_or_no(&opt, F5C_SCALE_EVENTS, longindex, "yes", 1);
        } else if (c == 0 && longindex == 36) { // print read names
            if (mode != 1) {
                ERROR("%s", "Option --print-read-names is available only in "
                            "eventalign");
                exit(EXIT_FAILURE);
            }
            ...
            ERROR("%s", "--samples not yet implemented. Submit a github "
                        "request if you need this feature.");
            exit(EXIT_FAILURE);
        } else if (c == 0 &&
                   longindex == 38) { // specify the version of the tsv output
                                      // for methylation (call-methylation only)
            opt.meth_out_version = atoi(optarg);
            if (mode != 0) {
                ERROR("%s", "Option --meth-out-version is available only in "
                            "call-methylation");
                exit(EXIT_FAILURE);
            }
            if (opt.meth_out_version < 1 || opt.meth_out_version > 2) {
                ERROR("--meth-out-version accepts only 1 or 2. You entered %d",
                      opt.meth_out_version);
                exit(EXIT_FAILURE);
            }
        } else if (c == 0 && longindex == 40) { // custom methylation k-mer
                                                // model
            if (mode != 0) {
                ERROR("%s", "Option --meth-model is available only in "
                            "call-methylation");
                exit(EXIT_FAILURE);
            }
    ...
    if (fastqfile == NULL || bamfilename == NULL || fastafile == NULL ||
        fp_help == stdout) {
        fprintf(fp_help,
                "Usage: f5c %s [OPTIONS] -r reads.fa -b alignments.bam -g "
                "genome.fa\n",
        ...
        fprintf(fp_help, "   -w STR                     limit processing to "
                         "genomic region string of format chr:start-end\n");
        fprintf(
            fp_help,
            "   -t INT                     number of processing threads [%d]\n",
            opt.num_thread);
        fprintf(fp_help,
                "   -K INT                     batch size (max number of reads "
                "loaded at once) [%d]\n",
                opt.batch_size);
        fprintf(fp_help,
        ...
        fprintf(fp_help, "   -x STR                     parameter profile to "
                         "be used for better performance (always applied "
                         "before other options)\n"); // Added option in help
        fprintf(fp_help,
                "                              e.g., laptop, desktop, hpc; see "
                "https://f5c.page.link/profiles for the full list\n");
        fprintf(fp_help,
                "   --iop INT                  number of I/O processes to read "
                "fast5 files [%d]\n",
                opt.num_iop);
        fprintf(fp_help,
                ...
                opt.min_mapq);
        fprintf(fp_help,
                "   --secondary=yes|no         consider secondary mappings or "
                "not [%s]\n",
...
#ifdef HAVE_CUDA
        fprintf(fp_help,
                "   --disable-cuda=yes|no      disable running on CUDA [%s]\n",
                (opt.flag & F5C_DISABLE_CUDA ? "yes" : "no"));
        fprintf(fp_help,
                "   --cuda-dev-id INT          CUDA device ID to run kernels "
                "on [%d]\n",
                opt.cuda_dev_id);
        fprintf(fp_help,
                "   --cuda-max-lf FLOAT        reads with length <= "
                "cuda-max-lf*avg_readlen on GPU, rest on CPU [%.1f]\n",
                opt.cuda_max_readlen);
        fprintf(fp_help,
                "   --cuda-avg-epk FLOAT       average number of events per "
                "kmer - for allocating GPU arrays [%.1f]\n",
                opt.cuda_avg_events_per_kmer);
        fprintf(fp_help,
                "   --cuda-max-epk FLOAT       reads with events per kmer <= "
                "cuda_max_epk on GPU, rest on CPU [%.1f]\n",
                opt.cuda_max_avg_events_per_kmer);
#endif
        fprintf(fp_help, "\nadvanced options:\n");
        fprintf(fp_help,
                "   --skip-ultra FILE          skip ultra long reads and write "
                "those entries to the bam file provided as the argument\n");
        fprintf(fp_help,
                "   --ultra-thresh INT         threshold to skip ultra long "
                "reads [%ld]\n",
                (long)opt.ultra_thresh);
        fprintf(fp_help,
                "   --skip-unreadable=yes|no   skip any unreadable fast5 or "
                "terminate program [%s]\n",
                (opt.flag & F5C_SKIP_UNREADABLE ? "yes" : "no"));
        fprintf(fp_help,
                "   --kmer-model FILE          custom nucleotide 6-mer model "
                "file (format similar to "
                "test/r9-models/r9.4_450bps.nucleotide.6mer.template.model)\n");
        if (mode == 0) {
            fprintf(fp_help,
                    "   --meth-model FILE          custom methylation 6-mer "
                    "model file (format similar to "
                    "test/r9-models/r9.4_450bps.cpg.6mer.template.model)\n");
            fprintf(fp_help,
                    "   --meth-out-version INT     methylation tsv output "
                    "version (set 2 to print the strand column) [%d]\n",
                    opt.meth_out_version);
        }
        if (mode == 1) {
            fprintf(fp_help, "   --summary FILE             summarise the "
                             "alignment of each read/strand in FILE\n");
            fprintf(
                fp_help,
                "   --sam                      write output in SAM format\n");
            fprintf(fp_help, "   --print-read-names         print read names "
                             "instead of indexes\n");
            fprintf(fp_help, "   --scale-events             scale events to "
                             "the model, rather than vice-versa\n");
            fprintf(fp_help, "   --samples                  write the raw "
                             "samples for the event to the tsv output\n");
        }
#ifdef HAVE_CUDA
        fprintf(fp_help, "   --cuda-mem-frac FLOAT      Fraction of free GPU "
                         "memory to allocate [0.9 (0.7 for tegra)]\n");
        ...
        fprintf(fp_help, "   --debug-break INT          break after processing "
                         "the specified no. of batches\n");
        fprintf(fp_help, "   --profile-cpu=yes|no       process section by "
                         "section (used for profiling on CPU)\n");
        fprintf(fp_help, "   --write-dump=yes|no        write the fast5 dump "
                         "to a file or not\n");
        fprintf(fp_help, "   --read-dump=yes|no         read from a fast5 dump "
                         "file or not\n");
    ...
    core_t *core = init_core(bamfilename, fastafile, fastqfile, tmpfile, opt,
                             realtime0, mode, eventalignsummary);
    ...
    if (mode == 0) {
        ...
        if (core->event_summary_fp != NULL) {
            fprintf(core->event_summary_fp,
                    "read_index\tread_name\tfast5_path\tmodel_"
                    "name\tstrand\tnum_events\t");
            fprintf(core->event_summary_fp,
                    "num_steps\tnum_skips\tnum_stays\ttotal_"
                    "duration\tshift\tscale\tdrift\tvar\n");
        }
        ...
        if (sam_output == 0) {
            emit_event_alignment_tsv_header(stdout, print_read_names,
                                            write_samples);
        } else {
...
#ifdef IO_PROC_NO_INTERLEAVE // If input, processing and output are not
                             // interleaved (serial mode)

    // initialise a databatch
    db_t *db = init_db(core);

    ret_status_t status = {core->opt.batch_size, core->opt.batch_size_bases};
    while (status.num_reads >= core->opt.batch_size ||
           status.num_bases >= core->opt.batch_size_bases) {
    ...
    while (status.num_reads >= core->opt.batch_size ||
        ...
        if (first_flag_p) { // if not the first time of the "process" wait for
                            // the previous "process"
            int ret = pthread_join(tid_p, NULL);
            NEG_CHK(ret);
            if (opt.verbosity > 1) {
                fprintf(stderr,
                        "[%s::%.3f*%.2f] Joined to processor thread %ld\n",
                        __func__, realtime() - realtime0,
        ...
        if (first_flag_pp) { // if not the first time of the post-process wait
                             // for the previous post-process
            int ret = pthread_join(tid_pp, NULL);
            NEG_CHK(ret);
            if (opt.verbosity > 1) {
                fprintf(stderr,
                        "[%s::%.3f*%.2f] Joined to post-processor thread %ld\n",
                        __func__, realtime() - realtime0,
                        cputime() / (realtime() - realtime0), (long)tid_pp);
            }
        }
        first_flag_pp = 1;
        ...
        if (opt.verbosity > 1) {
            fprintf(stderr,
                    "[%s::%.3f*%.2f] Spawned post-processor thread %ld\n",
                    __func__, realtime() - realtime0,
                    cputime() / (realtime() - realtime0), (long)tid_pp);
        }
    ...
    if (opt.verbosity > 1) {
        fprintf(stderr,
                "[%s::%.3f*%.2f] Joined to last post-processor thread %ld\n",
                __func__, realtime() - realtime0,
                cputime() / (realtime() - realtime0), (long)tid_pp);
    }
    ...
    fprintf(stderr,
            "[%s] total entries: %ld, qc fail: %ld, could not calibrate: %ld, "
            "no alignment: %ld, bad fast5: %ld",
            __func__, (long)core->total_reads, (long)core->qc_fail_reads,
            (long)core->failed_calibration_reads,
            (long)core->failed_alignment_reads, (long)core->bad_fast5_file);
    fprintf(stderr, "\n[%s] total bases: %.1f Mbases", __func__,
            core->sum_bases / (float)(1000 * 1000));

    fprintf(stderr, "\n[%s] Data loading time: %.3f sec", __func__,
            core->load_db_time);
    fprintf(stderr, "\n[%s]     - bam load time: %.3f sec", __func__,
            core->db_bam_time);
    fprintf(stderr, "\n[%s]     - fasta load time: %.3f sec", __func__,
            core->db_fasta_time);
    fprintf(stderr, "\n[%s]     - fast5 load time: %.3f sec", __func__,
            core->db_fast5_time);
    fprintf(stderr, "\n[%s]         - fast5 open time: %.3f sec", __func__,
            core->db_fast5_open_time);
    fprintf(stderr, "\n[%s]         - fast5 read time: %.3f sec", __func__,
            core->db_fast5_read_time);

    fprintf(stderr, "\n[%s] Data processing time: %.3f sec", __func__,
            core->process_db_time);

    if ((core->opt.flag & F5C_SEC_PROF) ||
        (!(core->opt.flag & F5C_DISABLE_CUDA))) {
        fprintf(stderr, "\n[%s]     - Events time: %.3f sec", __func__,
                core->event_time);
        ...
        if (!(core->opt.flag & F5C_DISABLE_CUDA)) {
            ...
            fprintf(stderr,
                    "\n[%s]           -additional cpu processing time (load "
                    "imbalance): %.3f sec",
    ...
    if (core->ultra_long_skipped > 0) {
        assert(tmpfile != NULL);
        WARNING("%ld ultra long reads (>%.1f kbases) were skipped.",
                (long)core->ultra_long_skipped,
                core->opt.ultra_thresh / 1000.0);
        fprintf(stderr,
                " Please run samtools index on '%s' followed by f5c with a "
                "larger -B on the CPU.\n",
                tmpfile);
    }
    ...
    if ((core->load_db_time - core->process_db_time) >
        (core->process_db_time * 0.2)) {
        INFO("Performance bounded by file I/O. File I/O took %.3f sec than "
             "processing",
    ...
    if (opt.verbosity > 0) {
        fprintf(stderr,
                "[%s] max-lf: %.2f, avg-epk: %.2f, max-epk: %.2f, K: %d, B: "
                "%ld, T: %d, Ultra: %ld, Align: %.3f, Diff: %.3f\n",
                __func__, opt.cuda_max_readlen, opt.cuda_avg_events_per_kmer,
                opt.cuda_max_avg_events_per_kmer, opt.batch_size,
                opt.batch_size_bases, opt.num_thread, opt.ultra_thresh,
